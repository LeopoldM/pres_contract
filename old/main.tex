...
How do consumers self-select into contracts in the retail electricity market?
\end{mainbox}
...
We study under which environment letting consumers choose their contract can lead to adverse selection.
\end{mainbox}
...
\item \boldrblue{A multidimensional adverse selection case}.

\vfill

\begin{itemize}
    \item Consumer reaction to contract prices is private information, so the retail cost of each consumer is based on their type.  
\end{itemize}


\vfill

\item \boldrblue{Optimal demand for contracts}.

\vfill

\begin{itemize} 
    \item Higher w.t.p. during low-price periods $\rightarrow$ RTP contract.
    \item Key ingredient: the set of marginal consumers consumer. 
\end{itemize}
...
\begin{itemize}
    \item When there are only competitive retailers on the market, only the RTP contract is offered $\rightarrow$ \boldrblue{No Adverse Selection}
    \item When the regulated firm offers the FP contract, there exists a demand for the FP contract $\rightarrow$ \boldrblue{Adverse Selection}
\end{itemize}

% \vfill

% \item {Extensions 1} - Incomplete information framework.
 
%     \vfill

% \begin{itemize}
%         \item \textit{Limiting information to the public firm might be efficient.}
%         \item \textit{Cross-subsidizing switching decisions require efficiency tradeoff.} 
% \end{itemize}

% \vfill

% \item {Extensions 2} -  Consumer appliance choices and renewable production.
...
\vfill
        ...
        % \begin{itemize}
 \item Drawn from a continuous uniform pdf. $f_t(v)$ and cdf. $F_t(v)$ with support $[\underline{v}_t,\bar{v}_t]$. 
...
\item Robust to $\displaystyle C(q) = \frac{b}{2} q^2$.
...
\begin{itemize}
    ...
    \item We take the characteristics of contracts as given.  \vfill
...
\item Either retailer can be perfectly competitive with private firms or a monopoly subject to price regulation.  
...
\begin{frame}{Demand for contracts and for electricity}
\begin{figure}
    \centering
    \includegraphics[width=0.8\linewidth]{figure/intro_zone.png}
    \caption{Example of how the demand is determined in the model given the distribution of types. Here, we assume there is only one outside option, which is not to consume.}
    \label{fig:intro_zone}
\end{figure}
...
\begin{figure}
    \centering
    \includegraphics[width=0.8\linewidth]{figure/intro_zone_qtt.png}
    \caption{Example of how the demand is determined in the model given the distribution of types. Here, we assume there is only one outside option, which is not to consume.}
...
\begin{figure}
    \centering
    \includegraphics[width=0.8\linewidth]{figure/intro_zone_mu.png}
    \caption{Example of how the demand is determined in the model given the distribution of types. Here, we assume there is only one outside option, which is not to consume.}
...
\item If the regulated monopoly offers the FP contract, then the objective of the public firm is to find two-part tariffs $\{p_r,A\}$ such that:
...
\item Under complete information, the fixed parts $A$ are individualized and become irrelevant: \boldrblue{Second-Best}. 
...
\begin{frame}{Case 1: FP contract only}

\begin{itemize}

\item Following \cite{joskow2006retail}. \vfill

\item If private retailers offer the contract, then the linear price is the eq. and takes the following form:
...
\begin{itemize}
    \item  \hyperlink{ramsey_init}{\beamergotobutton{Ramsey Intuitions}}
\end{itemize}

\vfill

\item Private Retailers: \boldrblue{Third-Best}, Regulated Monopoly: \boldrblue{Second-Best}. 
...
%      \caption{Each zone defines the type of consumers that choose each contract.}
...
\begin{equation*}
 \langle v_1-p_1 \rangle^+ + \langle v_2-p_2 \rangle^+ \geq \langle v_1-p_r \rangle^+ + \langle v_2-p_r \rangle^+
\end{equation*}

\vfill

\item Consumers who do not consume: $S_0 = [\ubar{v}_1,p_1]\times[\ubar{v}_2,p_r]$.
...
\begin{frame}{The indifferent consumers}


\begin{itemize}

\item Let $S^i(\textbf{p}^s,\textbf{p}^r)=\{\textbf{v}: U(\textbf{v},\textbf{p}^s)= U(\textbf{v},\textbf{p}^r)>0\}$ denote the set of types who are indifferent between the two contracts. 

\vfill

\item $S^i(\textbf{p}^r,\textbf{p}^r)$ is composed of two different subsets of consumers:

\vfill


\begin{flalign*}
\quad \quad S^i_a(\textbf{p}^s,\textbf{p}^r) \quad \quad = \quad \quad \begin{aligned}
 &\text{Consuming in period 1 with RTP} \\ 
 & \text{or} \\
 & \text{Consuming in period 2 with FP}
\end{aligned} &&
\end{flalign*}
...
\begin{flalign*}
\quad \quad  S^i_b(\textbf{p}^s,\textbf{p}^r) \quad \quad = \quad \quad 
\begin{aligned}
 & \text{Consuming in period 1 and 2 with $k$} \\ 
 & \text{or} \\
 & \text{Consuming in period 1 or 2 with $-k$}
\end{aligned}&&
\end{flalign*}
...
\begin{itemize}
    \item The relation between $\tilde{p}$ and $p_r$ determines the set $S^i_b(\textbf{p}^s,\textbf{p}^r)$.
\end{itemize}
...
\begin{lemma}
 \vspace{0.2cm}
Given a set of contract price $\textbf{p}^s=(p_1,p_2)$ and $\textbf{p}^r=(p_r,p_r)$,  \vspace{0.4cm}

\begin{itemize}
\item \textit{i)}  When $p_r < \tilde{p}$, consume during periods 1 and 2 with FP or consume during period 1 with RTP.  \vspace{0.4cm}
\item \textit{ii)} When $p_r > \tilde{p}$, consume during periods 1 and 2 with RTP or consume during period 2 with FP.  \vspace{0.4cm}
\item \textit{iii)}  When $p_r = \Tilde{p}$, the two previous indifferent consumers determine the set.
\end{itemize}
...
% \begin{frame}{Identifying the consumers' type}

% \begin{lemma}
% Given a set of contract price $\textbf{p}^s=(p_1,p_2)$ and $\textbf{p}^r=(p_r,p_r)$,  \vspace{0.5cm}
% \begin{itemize}
%  \item When $p_r < \Tilde{p}$,$\quad$ if $ \quad v_2  \quad < \quad \Phi_2(v_1) \quad  \rightarrow \quad $ RTP 
%  \vspace{0.5cm}
%  \item When $p_r > \Tilde{p}$,$\quad$ if $ \quad v_1  \quad <  \quad  \Phi_1(v_2) \quad 
%  \rightarrow \quad  $FP   \vspace{0.5cm}
%  \item When $p_r = \Tilde{p}$,$\quad$ if $ \quad \textbf{v}  \quad \in \quad   [v^{\Delta}_1,\bar{v}_1] \times [v^{\Delta}_2,\bar{v}_2] \quad 
%  \rightarrow \quad $ RTP $\equiv$ FP 
% \end{itemize}
% \end{lemma}
...
% \begin{itemize}\item[]
% \begin{itemize}
%     \item Low $v_1$ High $v_2$ choose the FP contract. \vfill
%     \item High $v_1$ Low $v_2$ choose the RTP contract.  \vfill
%     \item High $v_1$ High $v_2$ choice depends on $\tilde{p}$ and $p_r$.
% \end{itemize}
% \end{itemize}
...
\begin{frame}{The exogenous demand for contracts}\label{ExogenousDemand}

\begin{figure}
    \centering
    \includegraphics[width=1\linewidth]{figure/example_conv_cstc.png}
     \caption{Demand zone for the two contracts, depending on the consumer valuation. Note that the function $\phi$ is represented by the segment between $p_r$ and $v^{\Delta}_2$ when $p_r < \tilde{p}$ and between $p_r$ and $p_2$ when $p_r > \tilde{p}$.  \hyperlink{CompStat}{\beamergotobutton{Comp. Stat.}}} 
    \label{fig:example_conv_cstc2}
\end{figure}
...
\begin{frame}{What about the demand for electricity?}
...
% \begin{frame}{Some comparative statics}

% \begin{itemize}
%     \item We study how the demand for the two contracts change w.r.t:  \vfill

%     \begin{itemize}
%         \item relative demand parameters $\{\alpha,\beta\}$ \vfill
%         \item cost parameter: the average price $\Tilde{p}$ and the cost variation $d$ such that $p_1 = \Tilde{p} - d$ and $p_2 = \Tilde{p} + d$ 
%     \end{itemize}
% \end{itemize}
...
%  \begin{itemize}
%  \item The demand for RTP contracts ($\mu_{s}$) \textcolor{ForestGreen}{+} in $p_r$ and $d$  and \textcolor{red}{$-$}  in $\Tilde{p}$ and ($\beta$,$\alpha$).  \vspace{0.25cm}
%  \item The demand for FP contracts ($\mu_{r}$) \textcolor{red}{$-$} in $p_r$ and in $d$ and \textcolor{ForestGreen}{+} in $\Tilde{p}$  and ($\beta$,$\alpha$).
%  \end{itemize}
% \end{lemma}
...
%  \caption{Effects of a change in the wholesale price variation ($d$) and average value ($\tilde{p}$) on the demand for contracts. Black segments represent the initial s ate. The red segments and the values with $ '$ illustrate the change in the parameters.}
...
\begin{frame}{Private Firms offering the FP contract}\label{PrivateFirms1}

\begin{itemize}
    \item We study how private firms can set $p_r$ at the equilibrium.

    \vfill

    \item Two cases:  \vfill
    \begin{itemize}
        \item \hyperlink{qtt_elec1}{\beamergotobutton{Plot}} \vfill
        \item $p_r > \tilde{p} \quad \quad \rightarrow \quad \quad q^r_1 = 0 \quad q^r_2 > 0$ \vfill
        \item $p_r < \tilde{p} \quad \quad \rightarrow \quad \quad q^r_1 > 0 \quad q^r_2 > 0$ 
    \end{itemize}


\vfill

\item Which implies the following firms' profit

\begin{equation*}
 \pi^r(p_r) = \begin{cases}
 q^{r}_1(p_r,p_1,p_2) (p_r - p_1^w) + q^{r}_2(p_r,p_1,p_2) (p_r - p_2^w) &\quad \text{if} \quad p_r < \Tilde{p} \\\\
 q^{r}_2(p_r,p_1,p_2) (p_r - p_2^w) & \quad \text{if} \quad p_r > \Tilde{p}
 \end{cases}
\end{equation*}
    \end{itemize}
\end{frame}
...
\vfill
\begin{itemize}
\item When $p_r > \tilde{p}$, then consumers under FP only consume during period 2, which implies that $p_r = p_2$. $\quad$ \hyperlink{mass_elec}{\beamergotobutton{Plot}}
\vfill
...
\vfill
\begin{equation*}
     p_r = p_1^w \; \left(\frac{ q_1^r}{ q_1^r + q_2^r }\right) + p_2^w \;  \left(\frac{q_2^r}{ q_1^r + q_2^r}\right) > p_1^w \;  \left(\frac{1}{2}\right)  +  p_2^w \;  \left(\frac{1}{2}\right) = \tilde{p}
\end{equation*}
...
% \begin{itemize}
%     \item The regulated maximizes consumers surlus choosing the FP contract while satisfying a budget constraint.  \hyperlink{UniqueAPUWU}{\beamergotobutton{Example of unique A - 1}}   \hyperlink{UniqueAEq}{\beamergotobutton{Example of unique A - 2}}
%  \vfill
% \end{itemize}

% \begin{equation*}
%  \max_{p_r,A} CS(p_r) - A \hspace{0.5cm} \text{s.t.} \hspace{0.5cm} A + \sum_{t=1,2} \int_{S_t^r(p_r)} (p_r-p_t)f_t(v)dv\geq 0 
% \end{equation*}



\begin{itemize}
    \item The regulated maximizes consumers' surplus who choose the FP contract while satisfying a budget constraint. \vfill
    \item With complete information, it boils down to \boldrblue{maximizing the welfare of consumers under FP}.  \vfill
\end{itemize}

\begin{equation*}
 \max_{p_r} O(p_r) = \begin{cases}
 \displaystyle \sum_{t=1,2} \int_{S^{r}_t(p_r)}f_t(v)(v-p_t^w)dv \quad \quad & \text{if} \quad \quad p_r < \Tilde{p} \\\\
 \displaystyle \;\;\;\;\;\;\; \int_{S^{r}_2(p_r)}f_2(v)(v-p_2^w)dv \quad \quad & \text{if} \quad \quad p_r > \Tilde{p}
 \end{cases}
\end{equation*}
...
\begin{itemize}
    \item \hyperlink{CaseRamsey}{\beamergotobutton{Full FP Case}}, \hyperlink{RamseyPrices}{\beamergotobutton{Values}} \vfill
    \item No closed-form solution, so there is no clear ranking of which price is selected by the monopoly.
\end{itemize}
...
\begin{itemize}
    ...
    \item With only private retailers, \boldrblue{the First-Best is reached}. \vfill
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item This can leads to a $p_r < \textcolor{red}{p_r}$ \vfill 
    \end{itemize}
    \item But it implies more over-consumption from low-$v_2$ consumers. 
    \begin{itemize}
        \item Deadweight Loss from FP  \hyperlink{Plot_DW_DW}{\beamergotobutton{Plot}} 
    Net Deadweight Loss \hyperlink{Net_DW}{\beamergotobutton{Plot}} 
    \end{itemize} \vfill 
...
\begin{itemize}
    \item $\textbf{v}$ drawn from an exponential distribution function $f_t(v) = \lambda_t e^{- \lambda_t v }$ with support $[0,+\inf)$.  \vfill
    \item Main ingredient : $\lambda_2/\lambda_1$ \vfill
    \begin{itemize}
        \item Assuming that $\lambda_2 \geq \lambda_1$ then $\lambda_2/\lambda_1  \rightarrow 1$ implies a more symmetric distribution of $\textbf{v}$. \vfill
    \end{itemize}
    ...
    \begin{itemize}
        ...
        \item Two types of inefficiencies: \textbf{i)} production cost \textbf{ii)} adverse-selection
    \end{itemize}
\end{itemize}
...
\begin{figure}
    \centering
    \includegraphics[width=0.95\linewidth]{figure/DW_linc_exp_cos1.png}
    \caption{(adverse) selection of the public option can lead to a lower welfare compared to the full FP case. The surplus is even lower when excluding producer surplus differences.}
\end{figure}
...
% \toprule
% \multicolumn{1}{l|}{$p_r > \tilde{p} $} &\multicolumn{1}{c|}{ $p_r$ }                     & \multicolumn{1}{c|}{\cellcolor[HTML]{CBCEFB}{$d$}} & \multicolumn{1}{c|}{$\tilde{p}$}   & $\beta$                      & \multicolumn{1}{l}{|c {$\alpha$}} \\ \midrule
% \multicolumn{1}{l|}{$p_r$}              & \multicolumn{1}{c|}{+}     & \multicolumn{1}{c|}{\cellcolor[HTML]{CBCEFB}+}        & \multicolumn{1}{c|}{+}             & \multicolumn{1}{c|}{-}       & -                                 \\ \midrule
% \multicolumn{1}{l|}{$\mu_r$}            & \multicolumn{1}{c|}{-}     & \multicolumn{1}{c|}{\cellcolor[HTML]{CBCEFB}-}        & \multicolumn{1}{c|}{+}             & \multicolumn{1}{c|}{+}       & +                                 \\ \midrule
% \multicolumn{1}{l|}{$\mu_s$}            & \multicolumn{1}{c|}{+}     & \multicolumn{1}{c|}{\cellcolor[HTML]{CBCEFB}+}        & \multicolumn{1}{c|}{-}             & \multicolumn{1}{c|}{-}       & -                                 \\ \midrule
% \multicolumn{1}{l|}{$\mu_r(p_r)$}         & \multicolumn{1}{c|}{-}     & \multicolumn{1}{c|}{\cellcolor[HTML]{CBCEFB}-}        & \multicolumn{1}{c|}{?}             & \multicolumn{1}{c|}{+}       & +                                 \\ \midrule
% \multicolumn{1}{l|}{$\mu_sp_r$}         & \multicolumn{1}{c|}{+}     & \multicolumn{1}{c|}{\cellcolor[HTML]{CBCEFB}+}        & \multicolumn{1}{c|}{?}             & \multicolumn{1}{c|}{-}       & -                                 \\ \midrule
%                                         & \multicolumn{1}{l}{}       & \multicolumn{1}{l}{}                                  & \multicolumn{1}{l}{}               & \multicolumn{1}{l}{}         & \multicolumn{1}{l}{}              \\ \midrule
% \multicolumn{1}{l|}{$p_r < \tilde{p} $} & \multicolumn{1}{c|}{$p_r$} & \multicolumn{1}{c|}{$d$}                              & \multicolumn{1}{c|}{${\tilde{p}}$} & \multicolumn{1}{c|}{$\beta$} & $\alpha$                          \\ \midrule
% \multicolumn{1}{l|}{$p_r$}              & \multicolumn{1}{c|}{+}     & \multicolumn{1}{c|}{- $\rightarrow$ +}                & \multicolumn{1}{c|}{-}             & \multicolumn{1}{c|}{-}       & -                                 \\ \midrule
% \multicolumn{1}{l|}{$\mu_r$}            & \multicolumn{1}{c|}{-}     & \multicolumn{1}{c|}{-}                                & \multicolumn{1}{c|}{+}             & \multicolumn{1}{c|}{+}       & +                                 \\ \midrule
% \multicolumn{1}{l|}{$\mu_s$}            & \multicolumn{1}{c|}{+}     & \multicolumn{1}{c|}{+}                                & \multicolumn{1}{c|}{-}             & \multicolumn{1}{c|}{-}       & -                                 \\ \midrule
% \multicolumn{1}{l|}{$\mu_r(p_r)$}         & \multicolumn{1}{c|}{-}     & \multicolumn{1}{c|}{? $\rightarrow$ -}                & \multicolumn{1}{c|}{-}             & \multicolumn{1}{c|}{+}       & +                                 \\ \midrule
% \multicolumn{1}{l|}{$\mu_{s}(p_r)$}     & \multicolumn{1}{c|}{+}     & \multicolumn{1}{c|}{? $\rightarrow$ +}                & \multicolumn{1}{c|}{+}             & \multicolumn{1}{c|}{-}       & -                                 \\ \bottomrule
% \end{tabular}
% \end{table}
...
% \begin{tabular}{@{}lccccc@{}}
% \toprule
% \multicolumn{1}{l|}{$p_r > \tilde{p} $}& \multicolumn{1}{c|}{$p_r$} & \multicolumn{1}{c|}{$d$}               & \multicolumn{1}{c|}{\cellcolor[HTML]{CBCEFB}$\tilde{p}$} & $\beta$                      & \multicolumn{1}{l}{|c {$\alpha$}} \\ \midrule
% \multicolumn{1}{l|}{$p_r$}              & \multicolumn{1}{c|}{+}     & \multicolumn{1}{c|}{+}                 & \multicolumn{1}{c|}{\cellcolor[HTML]{CBCEFB}+}           & \multicolumn{1}{c|}{-}       & -                                 \\ \midrule
% \multicolumn{1}{l|}{$\mu_r$}            & \multicolumn{1}{c|}{-}     & \multicolumn{1}{c|}{-}                 & \multicolumn{1}{c|}{\cellcolor[HTML]{CBCEFB}+}           & \multicolumn{1}{c|}{+}       & +                                 \\ \midrule
% \multicolumn{1}{l|}{$\mu_s$}            & \multicolumn{1}{c|}{+}     & \multicolumn{1}{c|}{+}                 & \multicolumn{1}{c|}{\cellcolor[HTML]{CBCEFB}-}           & \multicolumn{1}{c|}{-}       & -                                 \\ \midrule
% \multicolumn{1}{l|}{$\mu_r(p_r)$}         & \multicolumn{1}{c|}{-}     & \multicolumn{1}{c|}{-}                 & \multicolumn{1}{c|}{\cellcolor[HTML]{CBCEFB}?}           & \multicolumn{1}{c|}{+}       & +                                 \\ \midrule
% \multicolumn{1}{l|}{$\mu_sp_r$}         & \multicolumn{1}{c|}{+}     & \multicolumn{1}{c|}{+}                 & \multicolumn{1}{c|}{\cellcolor[HTML]{CBCEFB}?}           & \multicolumn{1}{c|}{-}       & -                                 \\ \midrule
%                                         & \multicolumn{1}{l}{}       & \multicolumn{1}{l}{}                   & \multicolumn{1}{l}{}                                     & \multicolumn{1}{l}{}         & \multicolumn{1}{l}{}              \\ \midrule
% \multicolumn{1}{l|}{$p_r < \tilde{p} $} & \multicolumn{1}{c|}{$p_r$} & \multicolumn{1}{c|}{$d$}               & \multicolumn{1}{c|}{${\tilde{p}}$}                       & \multicolumn{1}{c|}{$\beta$} & $\alpha$                          \\ \midrule
% \multicolumn{1}{l|}{$p_r$}              & \multicolumn{1}{c|}{+}     & \multicolumn{1}{c|}{- $\rightarrow$ +} & \multicolumn{1}{c|}{-}                                   & \multicolumn{1}{c|}{-}       & -                                 \\ \midrule
% \multicolumn{1}{l|}{$\mu_r$}            & \multicolumn{1}{c|}{-}     & \multicolumn{1}{c|}{-}                 & \multicolumn{1}{c|}{+}                                   & \multicolumn{1}{c|}{+}       & +                                 \\ \midrule
...
\begin{figure}
    \centering
    \includegraphics[width=1\linewidth]{figure/example_conv_cqtt.png}
     \caption{When $p_r < \Tilde{p}$ (resp. $p_r > \tilde{p}$ ), some consumers that choose the FP contract (resp. RTP contract) consume in both periods, while consumers under the RTP contracts (resp. FP contract) consume only during period 1 (resp. period 2). \hyperlink{PrivateFirms1}{\beamergotobutton{Back}} }
...
\begin{frame}{The exogenous demand for contracts}\label{mass_elec}

\begin{figure}
    \centering
    \includegraphics[width=1\linewidth]{figure/example_conv_cstc.png}
     \caption{Demand zone for the two contracts, depending on the consumer valuation. Note that the function $\phi$ is represented by the segment between $p_r$ and $v^{\Delta}_2$ when $p_r < \tilde{p}$ and between $p_r$ and $p_2$ when $p_r > \tilde{p}$. \hyperlink{PrivateFirms2}{\beamergotobutton{Back}} }
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{frame}{Net deadweight loss}\label{Net_DW2}

\begin{figure}
    \centering
    \includegraphics[width=1 \linewidth]{figure/DW_pr1pr2.png}
     \caption{Welfare effect of consumer self-selection. Black values are for the self-selection case, and red values are for the fixed-price only case.\hyperlink{Extension1}{\beamergotobutton{Back}} }
    \label{fig:DW_pr1pr2}
\end{figure}
