...
\begin{itemize}
    \item \boldrblue{A selection market}: identity matters as cost depends on consumer types and contract choices.
        \vfill
    \item Consumers are \boldrblue{risk-neutral and fully rational} 
\end{itemize}
...
\node[firmbox=pubBlue, minimum width=5cm, minimum height=0.95cm,
  ...
  {\textcolor{pubBlue!15!black}{\textbf{\normalsize Public Firm}}\\[1pt]
   \textcolor{notegray}{\scriptsize regulated incumbent}};
...
\small
\begin{itemize}
    \item The public firm as a Universal Service Obligation provider, see Directive (EU) 2019/944
\end{itemize}
...
\begin{itemize}
    \item \boldrblue{Impossibility result}: FP contracts are not sustained at the equilibrium.     \vfill
    \begin{itemize}
    \item The option of a dynamic price contract unravels the FP market.  \vfill
    \item Holds for private firms and a self-financing public firm.
    \end{itemize}
    \vfill

\item \boldrblue{Adverse selection}: public funds/taxes sustain the FP contract, yet: \vfill
...
\item  Consumers differ in \boldrblue{willingness-to-pay} across consumption periods.
...
\begin{itemize}
 ...
 \item Extend single-type consumer models. \cite{joskow2006retail,Astier2021,ancel2025tariffs}
    \end{itemize}
...
\vfill
        ...
        \item Adverse Selection \cite{1970Akerlof}, price theory \cite{EinavAmy,Veiga2023}, product design \cite{Veiga2016}, competition \cite{Azevedo2017}
\vfill
...
\vfill
        ...
        % \begin{itemize}
\item Drawn from a pdf. $f_t(v_t)$ and cdf. $F_t(v_t)$ with support $[\underline{v}_t,\bar{v}_t]$, \boldrblue{independent across periods}: $f(\textbf{v}) = f_1(v_1)f_2(v_2)$.
...
\item Type $\textbf{v}$ gross payoff from consuming at price profile $\textbf{p}$ is:
...
\begin{itemize}
    \item We assume a set of retailers that purchase on the wholesale market at \boldrblue{spot price} $c_t$, with no additional retail cost. \vfill
    \begin{itemize}
    ...
    \begin{itemize}
        ...
        \item Dynamic contract\footnote{in equilibrium, the dynamic contract becomes a real-time-pricing contract}: $\textbf{p}^{s} =  (p_1,p_2)$  
    \end{itemize}
...
\item The equilibrium prices $(\mathbf{p}^k,A^k)$ are pinned down by \boldrblue{free entry}: zero profit on active contracts and no profitable deviation:
\vfill
    \begin{equation*}
...
\item $\lambda>0$ is the shadow cost of public funds.
...
{$u_1^{s}=[v_1-p_1]^+$\\[5pt]
$\Pi_1^{s}=(p_1-c_1)\mathbf{1}\{v_1\geq p_1\}$};

\node[lab] at (9.75,0.35)
{$u_2^{s}=[v_2-p_2]^+$\\[5pt]
$\Pi_2^{s}=(p_2-c_2)\mathbf{1}\{v_2\geq p_2\}$};

\node[lab] at (6.45,-2.05)
{$u_1^{r}=[v_1-p^r]^+$\\[5pt]
$\Pi_1^{r}=(p^r-c_1)\mathbf{1}\{v_1\geq p^r\}$};

\node[lab] at (9.75,-2.05)
{$u_2^{r}=[v_2-p^r]^+$\\[5pt]
$\Pi_2^{r}=(p^r-c_2)\mathbf{1}\{v_2\geq p^r\}$};
...
\begin{figure}
    ...
    \includegraphics[width=0.8\linewidth]{figure/fp_demand_multi.png}
     \caption{The canonical bundle case. $S_t$ denotes the set of consumers consuming only in period $t$, and $S_b$ in both periods. \hyperlink{EquilibriumSingle}{\beamerreturnbutton{Equilibrium}}  \hyperlink{Adverse}{\beamerreturnbutton{Adverse Selection}} }     
...
\begin{itemize}
    \item $A^k =0 $
    \item  $p_1 < p^r < p_2$
    \item $\bar{p} = \frac{1}{2}(p_1+p_2)$ \vfill
\end{itemize}
...
\begin{figure}
    \centering
    \includegraphics[width=0.60\linewidth]{figure/spot_demand_multi_sb_ppt.png}
    \caption{ The FP contract is introduced by a marginal downward deviation $p^r = p_2 - \epsilon$, with $\epsilon \geq 0$; the condition is evaluated at \boldrblue{$\epsilon = 0^+$}.}
...
\begin{proposition}
There is no equilibrium in which a positive mass of consumers strictly prefers an FP contract offered by \boldrblue{a private firm}.
\end{proposition}
...
\begin{figure}
    \centering
    \includegraphics[width=0.6\linewidth]{figure/Exemple_Private_u_ddeAC_ppt.png}
    \caption{Equilibrium with the single contract case, and market unraveling of the FP market with the RTP contract as an outside option}
\end{figure}
...
\begin{equation*}
    \underbrace{\Bigg. \int_{\M^\Delta_{a,b}|_{\epsilon = 0^+}} (v_2 - p_2) f_1(p_1 ) f_2(v_2 ) dv_2\Bigg.  }_{\text{marginal benefit}} > \underbrace{\Bigg. \lambda q_2^r(p_2)|_{\epsilon = 0^+}\Bigg. }_{\text{inframarginal cost}}  \label{con_exi_FP}
\end{equation*}
...
\begin{align*}
\partial_{p^r}\mathcal{C} :=  - \overbrace{\frac{\partial \phi}{\partial p^r} \int_{\M^\Delta_{a}} (\phi(v_1) - p^r-A^r) f_1(v_1) f_2(\phi(v_1))dv_1}^{\text{sorting effect \#1}}  \\
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        \item The following can be realized: \vfill
    \end{itemize}



\begin{center}
    W(\textcolor{blue}{Dynamic}: $p_t = c_t$ +  \textcolor{red}{Fixed-Price}: $p^r=p^r_{\ast}$)  { \textbf{  < }} $W$( \textcolor{red}{Fixed-Price}: $p_{fp}^r$)
\end{center}
...
\begin{figure}
    ...
    \caption{Consumer sorting in equilibrium under the single-contract and two-contract environments.}
\end{figure}
...
Change in deadweight loss between the two equilibria
...
\begin{figure}
    ...
    \caption{Equilibria under the single- and two-contract environments. Left panel: prices. Right panel: welfare. In the two-contract environment, the public firm uses either a two-part tariff or a linear price.}
\end{figure}
...
\begin{itemize}
    \item A \boldrblue{selection model} for retail electricity: can adverse selection arise when consumers choose their own contract? \vfill

    \item Self-selection yields two results: \vfill
    \begin{itemize}
        \item \boldrblue{Private firms}: impossibility, the FP market unravels. \vfill
        \item \boldrblue{Public firm}: the FP survives, but adverse selection lowers welfare. \vfill
    \end{itemize}

    \item \boldrblue{Beyond impossibility}: when can a private FP contract break even (make the curves cross)? \vfill
    \begin{itemize}
        \item A different selection pattern, e.g.\ risk aversion. \vfill
        \item A barrier to selection, $p^r > AC^r(p^r)$. \vfill
        \item A retail cost advantage, $\overline{AC}^r(p^r) < AC^r(p^r)$. \vfill
    \end{itemize}

\item \boldrblue{Looking ahead}: with \boldrblue{sophisticated firms}, can price discrimination make a private firm weigh marginal vs.\ inframarginal consumers like the public firm - price discrimination $\approx$ public firm?
...
\begin{figure}
    ...
    \caption{Demand is in line with intuition: decreasing for the FP and increasing for RTP.         \hyperlink{demand1}{\beamerreturnbutton{Go Back}}}
    \label{fig:Demand_PR}
...
where $T = q^s + \tau^s \delta_{\tau}(q^s)$, $P^r = p^r + \tau^r$
